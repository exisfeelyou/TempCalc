...
async def show_instructions(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "🤖 <b>Выберите раздел инструкции:</b>",
        reply_markup=reply_markup,
        parse_mode="HTML"
    )
...
<b>Шаг 1: Ввод номера реактора</b>
• Поддерживаемые форматы:
<blockquote>
- Буквенные: <code>ТМ-Н</code>, <code>ТМН</code>, <code>тм-н</code>, <code>тмн</code>, <code>Тм-н</code>, <code>Тмн</code>
- Цифровые: <code>Y-Y</code>, <code>YY-Y</code>, <code>YY</code>, <code>YYY</code>
- Примеры: <code>1-1</code>, <code>11-1</code>, <code>11</code>, <code>111</code>
</blockquote>

<b>Шаг 2: Установка рабочих диапазонов (опционально)</b>
• После ввода номера реактора вы можете:
<blockquote>
- <b>Установить особые диапазоны для этого реактора</b>
- Использовать общие диапазоны
- Пропустить этот шаг и использовать стандартные диапазоны
</blockquote>

<b>Шаг 3: Ввод температур</b>
• Форматы ввода:
<blockquote>
- Базовый: <code>[три текущих температуры] [одна температура задания]</code>
- Расширенный: <code>[три текущих температуры] [три температуры задания]</code>
• Примеры:
...
<b>Шаг 4: После расчета корректировок</b>
• Бот покажет:
<blockquote>
- Номер реактора
- Текущие температуры по зонам (Б Ц Д)
- Заданные температуры
- Используемые диапазоны (общие или особые)
- Необходимые корректировки для каждой зоны
- Предполагаемые температуры после корректировок
</blockquote>

• Доступные действия:
<blockquote>
- <b>"Завершить вывод канала"</b> - если работа с реактором закончена
- <b>"Отредактировать вводимые температуры"</b> - если нужно ввести новые текущие температуры
</blockquote>

<b>Редактирование температур реактора:</b>
• При нажатии <b>"Отредактировать вводимые температуры"</b> отобразится:
<blockquote>
- Текущий номер реактора
- Текущие температуры по зонам
- Температуры задания
</blockquote>

• Введите новые текущие температуры:
<blockquote>
- Формат: <code>1008.5 1003.7 1001.2</code>
- Или: <code>1008,5 1003,7 1001,2</code>
- Вводятся только три значения текущих температур
- Температуры задания останутся прежними
</blockquote>
"""
...
• Для каждого реактора при нажатии отображается:
<blockquote>
- Номер реактора
- Текущие температуры по зонам (Б Ц Д)
- Температуры задания
- Используемые диапазоны (общие или особые)
- Необходимые корректировки по зонам
- Прогноз температур после корректировок
</blockquote>

• Доступные действия для каждого реактора:
<blockquote>
- <b>"Завершить вывод канала"</b> - удаляет реактор из активных выводов
- <b>"Отредактировать вводимые температуры"</b> - позволяет ввести новые текущие температуры
</blockquote>
"""
    keyboard = [[InlineKeyboardButton("◀️ Назад к разделам", callback_data="back_to_instructions")]]
    reply_markup = InlineKeyboardMarkup(keyboard)
...
<blockquote>
- Нажмите на кнопку зоны (<b>Б</b>, <b>Ц</b> или <b>Д</b>)
- Введите два числа: <code>мин макс</code>
- Пример: <code>+2 0</code>
</blockquote>

<b>Установка всех диапазонов сразу:</b>
• Нажмите <b>"Установить для всех зон сразу"</b>
• Введите 6 чисел в формате:
<blockquote>
<code>Б_мин Б_макс Ц_мин Ц_макс Д_мин Д_макс</code>
• Пример: <code>+2 0 +1 -1 0 -1</code>
</blockquote>

<b>Особые диапазоны для реактора:</b>
• В списке диапазонов нажмите на реактор для просмотра его особых диапазонов
• При просмотре особых диапазонов доступны действия:
<blockquote>
- <b>"Изменить диапазоны"</b> - установка новых особых диапазонов
- <b>"Удалить особые диапазоны"</b> - возврат к общим диапазонам
</blockquote>

• При просмотре показываются:
<blockquote>
- Текущие особые диапазоны для зон Б, Ц, Д
- Минимальные и максимальные значения для каждой зоны
</blockquote>
"""
    keyboard = [[InlineKeyboardButton("◀️ Назад к разделам", callback_data="back_to_instructions")]]
    reply_markup = InlineKeyboardMarkup(keyboard)
